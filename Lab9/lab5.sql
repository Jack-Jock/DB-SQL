...
--
-- Хост: 127.0.0.1
-- Время создания: Май 28 2023 г., 13:57
...
-- Функции
--
CREATE DEFINER=`root`@`localhost` FUNCTION `Funct_1` (`year_date` INT) RETURNS FLOAT  RETURN (
...
(36, 5121, 'Yes', 'Апаратні засоби мультимедіа.Відеосистема PC', 19.5, 1, 200, 1, '2023-07-24', 5000, 1, 1),
(37, 5121, 'Yes', 'Апаратні засоби мультимедіа.Відеосистема PC', 15.5, 1, 300, 1, '2023-07-24', 5000, 1, 1),
...
DELIMITER ;
DELIMITER $$
CREATE TRIGGER `trigg_6` BEFORE DELETE ON `book` FOR EACH ROW BEGIN
...
DELIMITER ;
DELIMITER $$
CREATE TRIGGER `trigg_7` BEFORE UPDATE ON `book` FOR EACH ROW BEGIN 
